...
//    }
    ...
    protected void onDraw(Canvas canvas){
        ...
        int wideKeyWidth = (int) Math.floor(0.052 * getWidth());
        int smallKeyWidth = (int) Math.floor(0.038 * getWidth());

        int boxWidth = 64;
        int boxHeight = 128;

        int currentWidth = 0;
        int drawHeight = (int) (getHeight() * 0.75);
        ...
        for (int i = 0; i < 24; i++){
            canvas.drawRect(currentWidth, drawHeight, currentWidth + boxWidth, drawHeight + boxHeight, paint);
            if (isWide[i]){
